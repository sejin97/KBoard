...
public class PrincipalDetails implements UserDetails, OAuth2User {
    ...
    @Override
    public Collection<? extends GrantedAuthority> getAuthorities() {        //권한들을 리턴하는 목록
        System.out.println("getAuthorities() 호출");

        Collection<GrantedAuthority> collect = new ArrayList<>();

        List<Authority> list = userService.selectAuthoritiesById(user.getId()); //db에서 로그인한 user 의 권한들을 읽어오기
