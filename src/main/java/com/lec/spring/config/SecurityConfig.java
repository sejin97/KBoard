...
//        return web -> web.ignoring().anyRequest();  //어떠한 requewt 도 Security 가 무시함
//    }
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        return http
                .csrf(csrf -> csrf.disable())   //CSRF 비활셩화
                /**********************************************
                ...
                .authorizeHttpRequests(auth -> auth
                        // URL 과 접근권한 세팅(들)
                        // ↓ /board/detail/** URL로 들어오는 요청은 '인증'만 필요. 권한 필요 없음
                        .requestMatchers("/board/detail/**").authenticated() // requestMathers로 /board/detail/이 들어간 모든 주소를 통제하고 .authenticated() 함수로 인증된 사람만 들어올 수 있게 한다.
                ...
                .formLogin(form -> form
                                .loginPage("/user/login")   //만약에 로그인이 필요하면 주소로 보내주세요 // 로그인 필요한 상황 발생시 매개변수의 url (로그인 폼) 으로 request 발생
...
//                        .usernameParameter("aaa")   //기본 name = "username"이다, login 폼에 있는 name이랑 같다.
//                        .passwordParameter("bbb")   // 기본 password = "password", login 폼에 있는 name이랑 같다.
...
//                                .logoutSuccessUrl("/home")  //로그아웃 성공 후 redirect url
                                .invalidateHttpSession(false)   //session invalidate 수행 못시키게함
                                // 이따가 CustomLogoutSuccessHandler 에서 꺼낼 정보가 있기 때문에
                                // false 로 세팅한다

//                                .deleteCookies("JSESSIONID") // 쿠기 제거할 때 사용
                ...
                .oauth2Login(httpSecurityOAuth2LoginConfigurer -> httpSecurityOAuth2LoginConfigurer
                        .loginPage("/user/login") // 로그인 페이지를 기존과 동일하게 한다.
                        // ↑ 구글 로그인 완료된 뒤에 후처리가 필요하다!

                        // code 를 받아오는 것이 아니라, 'AccessToken' 과 사용자 '프로필정보'를 한번에 받아온다
                        .userInfoEndpoint(userInfoEndpointConfig -> userInfoEndpointConfig
                                // 인증서버의 userinfo endpoint 를 설정
                                .userService(principalOauth2UserService)  //userService(OAuth2UserService)를 등록해줘야 한다.
                        )
